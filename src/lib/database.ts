...
//die wird jedesmal gerufen, wenn eine Änderung in der Datenbank vorgenommen wurde und die Anzeige auf der Webseite aktualisiert werden muss.
...
export async function getSubFolder(ogFolder : Folder) {
    ogFolder.child = []
    let pathArray: string[] = ogFolder.path.split('/')                                                            //Pfade werden in dem Vormat: folder/subfoler/subfoler/file angegeben. Hier wird dieser dann in die einzelnen Ordner geteilt
    let line = 0;
    let max = lines.length;
    let path = 0;
    while (line < max) {
        let folderSize = +lines[line].substring(lines[line].lastIndexOf('~') + 1)          
        if (lines[line].substring(1, lines[line].lastIndexOf('~')) === pathArray[path]) {                         //Die Datenbank wird von oben durchgegangen und gegprüft, ob der Ordner teil des Dateipfades ist. Ist dies nicht der Pfeil, wird der Inhalt des Ordners übesprunge, was das ganze effektiver macht.
            max = line + folderSize
...
//fügt eine Datei sammt messageIDs zur Datenbank hinzu und addiert die Menge der hinzugefügten Dateien, zu allen Foldersizes der Folder, in denen die Datei gespeichert ist.
...
//funktionert gleich wie die addFile funktion nur ohne messageIDs 
